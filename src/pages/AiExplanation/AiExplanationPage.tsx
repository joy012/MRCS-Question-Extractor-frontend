...
import {
  useGetAiExplanationLogsQuery,
  useGetAiExplanationStatisticsQuery,
  useGetAiExplanationStatusQuery,
  useStartAiExplanationMutation,
  useStopAiExplanationMutation
} from '../../services/queries/useAiExplanation';
import { AiExplanationLogs } from './components/AiExplanationLogs';
...
const AiExplanationPage = () => {
  ...
  const stopMutation = useStopAiExplanationMutation();
  ...
  // Format time since last explanation
  ...
  const handleStartExplanation = () => {
    startMutation.mutate({ model: 'meditron' });
  };
            ...
            <div className="flex gap-3">
              {isProcessing ? (
                ...
                >
                  ...
                  {startMutation.isPending ? 'Starting...' : 'Start AI Explanations'}
                </Button>
              )}
            </div>
          </div>
      ...
      {isProcessing && (
        <Alert className="border-blue-200 bg-blue-50/50">
          ...
          <AlertDescription className="text-blue-800">
            AI explanation process is running. Processing {status?.processedQuestions || 0} of {status?.totalQuestions || 0} questions.
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-gray-600">With Explanations</span>
                </div>
              </div>
            </CardHeader>
            <CardContent className="pt-0">
              <div className="text-2xl font-bold text-green-600">
                {statistics?.questionsWithExplanation?.toLocaleString() || 0}
              </div>
              <p className="text-xs text-green-600 mt-1">
                {statistics?.explanationCoverage ? `${statistics.explanationCoverage.toFixed(1)}%` : '0%'} coverage
              </p>
            </CardContent>
          </Card>
        )}
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-gray-600">Need Explanations</span>
                </div>
              </div>
            </CardHeader>
            <CardContent className="pt-0">
              <div className="text-2xl font-bold text-orange-600">
                {statistics?.questionsWithoutExplanation?.toLocaleString() || 0}
              </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription className="text-gray-600">
                  AI explanation generation in progress
                </CardDescription>
              </div>
            </div>
          </CardHeader>
      ...
      <Card className="border-0 shadow-lg bg-gradient-to-br from-gray-50 to-gray-100/50">
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-lg font-semibold text-gray-900">AI Explanation Management</CardTitle>
              <CardDescription className="text-gray-600">
                Monitor and manage AI-powered question explanations
              </CardDescription>
            </div>
          </div>
        </CardHeader>
                  ...
                  <>
                    <div className="flex items-center justify-between p-3 bg-gradient-to-r from-indigo-50 to-indigo-100/50 border border-indigo-200/50 rounded-xl">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-medium text-gray-900">Last Explanation</span>
                      </div>
                      <Badge variant="secondary" className="bg-indigo-100 text-indigo-800">
                        {formatTimeSince(statistics?.lastExplanationAdded)}
                      </Badge>
                    </div>
